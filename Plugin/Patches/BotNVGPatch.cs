...
{
    internal class BotNVGPatch : ModulePatch
    {
        ...
        {
            if (!WildSpawnTypeExtensions.IsLegion(__instance.BotOwner_0.Profile.Info.Settings.Role))
            {
                return false;
            }

            if (__instance.StopTryingMove)
            {
                return true;
            }
            ...
            {
                __instance.BotOwner_0.GetPlayer.InventoryController.TryRunNetworkTransaction(
                    __instance.NightVisionItem.Togglable.Set(false, true, false),
                    null
                );
            }
