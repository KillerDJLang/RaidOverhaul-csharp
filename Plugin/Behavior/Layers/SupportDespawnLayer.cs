...
{
    internal class SupportDespawnLayer : CustomLayer
    {
        private SupportBotManager _manager;
        private Type _nextAction;
        private Type _lastAction;

        public SupportDespawnLayer(BotOwner botOwner, int priority)
            : base(botOwner, priority) { }

        public override string GetName()
        {
            return "SupportDespawn";
        }
        ...
        public override bool IsActive()
        {
            ...
            if (_manager == null || !_manager.IsActive)
            {
                return false;
            }

            if (!_manager.IsDespawning)
            {
                return false;
            }

            _nextAction = typeof(SupportRetreatAction);
            return true;
        }
